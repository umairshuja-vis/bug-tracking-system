...
import { Box, Breadcrumbs, CircularProgress, Typography, TextField, Paper, Grid, Table, Button, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, IconButton, Tooltip, Avatar, InputAdornment, MenuItem, useTheme, Menu, Divider, Select } from '@mui/material'
import { useBugs } from '@/hooks/useBugs'
import { useProjects } from '@/hooks/useProjects'
import { useStatusStyle } from '@/hooks/useStatusStyle'
import Link from 'next/link'
import { SearchIcon, Trash2, MoreVertical, Calendar, ChevronRight, LayoutGrid, TextAlignJustify, ArrowDownWideNarrow, ArrowUpWideNarrow } from 'lucide-react'
...
export default function BugsPage( { params }: BugsPageProps ) {
    ...
    if ( loading ) return <Box sx={ { px: 15, py: 4 } }><CircularProgress size={ 40 } />
    </Box>
    ...
    return (
        <Box sx={ { px: '17vw', py: 2 } }>
            ...
            <Box sx={ {} }>
                <Breadcrumbs sx={ { mb: 2 } } separator={ <ChevronRight size={ 16 } /> }>
                    <Link href="/projects" style={ { color: theme.palette.text.secondary } }>Projects</Link>
            ...
            >
                    ...
                    getStatusOptions( selectedBug.type ).map( ( status ) => {
                        ...
                        return (
                            <MenuItem
                                ...
                                <Box sx={ { bgcolor: statusStyle.bg, color: statusStyle.text, px: 1, py: 0.4, lineHeight: '20px', borderRadius: '4px', textTransform: 'capitalize', fontWeight: 500, fontSize: '11.5px' } }>
                                    { status }
                                </Box>
                            </MenuItem>
                        )
                ...
                { selectedBug && isCreator( selectedBug.bug_creator ) && (
                    <MenuItem
                        ...
                        sx={ { fontSize: '14px', py: 1, px: 2, alignItems: 'center', justifyContent: 'space-between', color: 'error.main' } }>
                        Delete
                        <Trash2 size={ 13.5 } />
                    </MenuItem>
                ) }
